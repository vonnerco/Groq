...
# streamlit-ace: graceful fallback if not installed
try:
...
ACE_THEMES = [
    ...
    "xcode", "textmate", "solarized_dark", "solarized_light", "terminal",
]

ACE_LANGUAGES = [
    "python", "javascript", "typescript", "html", "css", "json",
    "xml", "yaml", "markdown", "sh", "sql", "java", "text", "ini",
]


def ace_language_for_file(filename: str) -> str:
...
def render_ace_editor_panel():
    ...
    if not ACE_AVAILABLE:
        st.warning(
            "streamlit-ace is not installed. Run:\n\n"
    ...
    st.markdown("### 🖊️ Code Editor (Ace)")

    # ── Toolbar row ────────────────────────────────────────────────────────────
    ace_toolbar_cols = st.columns([2, 2, 1, 1, 1, 1])

    with ace_toolbar_cols[0]:
        chosen_language = st.selectbox(
            "Language",
            options=ACE_LANGUAGES,
            index=ACE_LANGUAGES.index(st.session_state.ace_editor_language)
            if st.session_state.ace_editor_language in ACE_LANGUAGES
            else 0,
    ...
    with ace_toolbar_cols[1]:
        chosen_theme = st.selectbox(
            "Theme",
            options=ACE_THEMES,
            index=ACE_THEMES.index(st.session_state.ace_editor_theme)
            if st.session_state.ace_editor_theme in ACE_THEMES
            else 0,
    ...
    with ace_toolbar_cols[4]:
        ...
        uploaded_file_names = [
            item.get("label") or item.get("original_name") or ""
            for item in st.session_state.get("uploaded_files", [])
            if item.get("path") and os.path.exists(item.get("path", ""))
        ]
        if uploaded_file_names:
            load_file_choice = st.selectbox(
                "Load file",
                options=["— pick —"] + uploaded_file_names,
                key="ace_load_file_select",
                label_visibility="collapsed",
            )
            if load_file_choice and load_file_choice != "— pick —":
                target = find_uploaded_file_by_query(load_file_choice)
                if target and os.path.exists(target.get("path", "")):
                    st.session_state.ace_editor_content = read_text_file(target["path"])
                    st.session_state.ace_editor_language = ace_language_for_file(
                        target.get("original_name") or load_file_choice
                    )
                    st.rerun()
        else:
            st.caption("No files loaded")

    with ace_toolbar_cols[5]:
        ace_save_name = st.text_input(
            "Save as",
            placeholder="filename.py",
            key="ace_save_filename",
            label_visibility="collapsed",
        )

    # ── Editor widget ──────────────────────────────────────────────────────────
    editor_content = st_ace(
        value=st.session_state.ace_editor_content,
        language=st.session_state.ace_editor_language,
        theme=st.session_state.ace_editor_theme,
        key="ace_editor_widget",
        height=400,
        font_size=14,
        tab_size=4,
        show_gutter=True,
    ...
    with save_row_cols[1]:
        if st.button("💾 Save to disk", use_container_width=True, key="ace_save_disk_btn"):
            fname = (ace_save_name or "").strip()
            if not fname:
                st.warning("Enter a filename in the 'Save as' box above first.")
            else:
                try:
                    base_dir = os.path.dirname(os.path.abspath(__file__))
                    save_path = os.path.join(base_dir, fname)
                    with open(save_path, "w", encoding="utf-8") as fh:
                        fh.write(st.session_state.ace_editor_content or "")
                    st.success(f"Saved to `{save_path}`")
                except Exception as exc:
                    st.error(f"Save failed: {exc}")

    with save_row_cols[2]:
        dl_name = (ace_save_name or "").strip() or "editor_content.txt"
        st.download_button(
            "⬇ Download",
            data=(st.session_state.ace_editor_content or "").encode("utf-8"),
            file_name=dl_name,
            mime="text/plain",
            use_container_width=True,
            key="ace_download_btn",
        )

    # ── Run code (Python only) ─────────────────────────────────────────────────
    if st.session_state.pop("ace_run_requested", False):
        code = st.session_state.ace_editor_content or ""
        if not code.strip():
            st.info("Editor is empty — nothing to run.")
        elif st.session_state.get("safe_mode"):
            st.warning("Safe mode is ON — code execution is disabled.")
        elif st.session_state.ace_editor_language != "python":
            st.info(f"Only Python execution is supported in the editor (current language: {st.session_state.ace_editor_language}).")
        else:
            st.markdown("**Output:**")
            stdout_out, stderr_out = execute_code(code)
            if stdout_out:
...
# ── Code Editor (Ace) expander ─────────────────────────────────────────────────
with st.expander("🖊️ Code Editor", expanded=st.session_state.get("ace_editor_open", False)):
    render_ace_editor_panel()
